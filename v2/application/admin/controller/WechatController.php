...
//include __DIR__ . '/../../../vendor/vendor/autoload.php';
...
class WechatController extends Controller
{
    ...
    public function init()
    {
        ...
        vendor("dodgepudding.wechat-php-sdk.Thinkphp.EasyWechat#class");
        $config = model('WxConfig')->find()->toArray();

        $options = array(
            'token' => 'zylcy', //填写你设定的key
            'encodingaeskey' =>'ts47B2ziVxl69FLVuCqHPFCmDFVaX00cpdymrKjiMRF', //填写加密用的EncodingAESKey
            'appid' => 'wx9f4368caa4218285', //填写高级调用功能的app id
            'appsecret' => 'a465a6716b511c28051979cbb777f0e6' //填写高级调用功能的密钥
        );

        self::$weObj = new \EasyWechat ($options);
    }
    ...
    public function index()
    {
       ...
       error_log(__FILE__.'['.__LINE__."]");
        $type = self::$weObj->getRev()->getRevType();
         ...
         error_log(__FILE__.'['.__LINE__."] {$type}");
    ...
    error_log(__FILE__.'['.__LINE__."] {$key}");
        $reply = model('WxReply')->where('key',$key)->find();
            error_log(__FILE__.'['.__LINE__."] ".  json_encode($reply));
            ...
            if ($reply["type"] == "news") {
                $newsArr = array(
                    array(
                        ...
                        'PicUrl' => self::$appUrl . '/..' . $reply["file_id"],
                        'Url' => $reply["url"]
                    )
                );
                    error_log(__FILE__.'['.__LINE__."] ");
    ...
    public function toKeyUnknow($key)
    {
        ...
        $kfList=self::$weObj->getCustomServiceKFlist();
           error_log(__FILE__.'['.__LINE__."] {$key}" .  json_encode($kfList));
           if(empty($kfList)){
               
                 self::$weObj->text('sleep')->reply();
                                  return;
           }
