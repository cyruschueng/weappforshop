...
class LoginxController extends BasexxController {
    ...
    public function post(){
        $uname = I('request.adm_name','','htmlspecialchars');
        $pass = I('request.adm_password','','htmlspecialchars');
     $arr['status'] = 1;
        ...
        if(empty($uname) || empty($pass)){
            return $this->echomsgerr('请先提交登陆信息');
        }

        $user = M('short_video_admin')->where(array('uname'=>$uname))->where(array('is_del' => 0))->find();
        if(!$user){
           return $this->echomsgerr('登陆失败');
        }

        if($user['status'] != 1){
             return $this->echomsgerr('账户正在审核中');
        }
