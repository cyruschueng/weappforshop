            ...
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <div class="ibox-content">
                        <div class="">
                            <!--<a href="<?php echo U('/short_vbanner/edit');?>" target="_self" class="btn" style="background: #e00214;color: #fff;">添加banner</a>-->
                        </div>
                    </div>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="white-space: nowrap; text-align: center">
                                    <button type="button" class="btn" id="delete-short_vbanner" style="background: #e00214;color: #fff;">删除</button>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr class="gradeX" url="<?php echo ($item["url"]); ?>" id="<?php echo ($item["id"]); ?>">
                                    ...
                                    <td><?php if($item['platform'] == 1): ?>H5<?php elseif($item['platform'] == 2): ?>App<?php else: ?>所有<?php endif; ?></td>
                                    <td><?php if($item['jump_type'] == 1): ?>视频详情<?php elseif($item['jump_type'] == 2): ?>我要上电视<?php else: ?>普通链接<?php endif; ?></td>
                                    <td><?php echo (mb_substr($item["url"],0,30)); ?></td>
                                    <td>
                                        <?php if( $item["image"] ): ?><img src="<?php echo ($item["image"]); ?> " class="view_img " data-url="<?php echo ($item["image"]); ?> " style="max-width: 100px; max-height: 80px " /><?php endif; ?>
                                    </td>
                                    <td><?php echo ($item["order"]); ?></td>
                                    <td><?php echo (date('Y-m-d H:i:s',$item["begin_time"])); ?></td>
                                    <td><?php echo (date('Y-m-d H:i:s',$item["end_time"])); ?></td>
                                    <td class="center " style="white-space: nowrap; ">
                                        <a href="<?php echo U( '/short_vbanner/edit', array( 'id'=>$item['id']));?>"><i class="fa fa-check text-navy"></i> 编辑</a>
                                    </td>
...
<script src="/Public/js/active-msdt.js?v=1.0.0"></script>
    ...
    <script type="text/javascript">
        $('select[name="search"]').change(function() {
            window.location.href = "<?php echo U('/short_vbanner/index');?>&class=" + $(this).val();
        });

        $("#delete-short_vbanner").click(function() {
            if (!confirm("确定要删除吗？")) {
                return false;
            }
            ...
            window.location.href = "/index.php?s=/short_vbanner/delete/list/" + idList.toString() + ".html";
        });
    </script>
