            ...
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <div class="ibox-content">
                        <div class="">
                            <!--<a href="<?php echo U('/shop_vbanner/edit');?>" target="_self" class="btn" style="background: #e00214;color: #fff;">添加banner</a>-->
                        </div>
                    </div>
                    <table class="table table-striped table-bordered table-hover " id="editable">
                        ...
                        <tbody>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><td><?php echo (mb_substr($item["title"],0,30)); ?></td>
                                    <td>
                                    
                                        <?php if( $item["picurl"] ): ?><img src="<?php echo ($item["picurl"]); ?> " class="view_img " data-url="<?php echo ($item["picurl"]); ?> " style="max-width: 100px; max-height: 80px " /><?php endif; ?>
                                    </td>
                                    <td><?php echo ($item["order"]); ?></td>
                                    <td><?php echo (date('Y-m-d H:i:s',$item["begin_time"])); ?></td>
                                    <td><?php echo (date('Y-m-d H:i:s',$item["end_time"])); ?></td>
                                    <td class="center " style="white-space: nowrap; ">
                                        <a href="<?php echo U( '/shop_vbanner/edit', array( 'id'=>$item['id']));?>"><i class="fa fa-check text-navy"></i> 编辑</a>
                                    </td>
...
<script src="/Public/js/active-msdt.js?v=1.0.0"></script>
    ...
    <script type="text/javascript">
        $('select[name="search"]').change(function() {
            window.location.href = "<?php echo U('/shop_vbanner/index');?>&class=" + $(this).val();
        });
