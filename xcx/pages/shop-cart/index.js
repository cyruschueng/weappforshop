...
Page({
    ...
    toPayOrder: function() {
        ...
        for (var i = 0; i < shopList.length; i++) {
            ...
            carShopBeanall[shopList[i].goodsId] = shopList[i];
            if (carShopBean.active == false) { doneNumber++;
            ...
            if (true||!carShopBean.propertyChildIds || carShopBean.propertyChildIds == "") {
                wx.request({
                    url: 'https://small.kuaiduodian.com/' + '/shop/goods_detail' + '?mallname=' + app.globalData.subDomain,
                    data: {
                        id: carShopBean.goodsId
                    },
                    success: function(res) {
                      carShopBean = carShopBeanall[res.data.data.id];
                        doneNumber++;
                        //有规格  没有设置规格 不提示
                        if (false && res.data.data.properties) {
                            wx.showModal({
                                title: '提示',
                ...
                wx.request({
                    url: 'https://small.kuaiduodian.com/' + '/shop/goods_price' + '?mallname=' + app.globalData.subDomain,
                    data: {
